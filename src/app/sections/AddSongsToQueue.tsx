    ...
    const image =
        typeof song.image === "string"
            ...
            : Array.isArray(song.image)
                ? (song.image as { quality: string; url: string }[]).find(img => img.quality === "500x500")?.url || ""
                : "";

    if (!image) {
        console.warn("Missing image for song:", song);
        return null;
    }

    return (
        <motion.li drag="x" dragElastic={0.2} dragConstraints={{ left: 0, right: 0 }} style={{ x }} dragTransition={{ power: 0.3, timeConstant: 150 }} whileHover={{ scale: 1.05 }} transition={{ duration: 0.2 }} onDragStart={handleDragStart} onDrag={handleDrag} onDragEnd={(e, info) => handleDragEnd(e, info, x, onDragComplete)} onClick={handleClick} className={`flex items-center gap-3 min-h-20 p-2 md:w-[400px] md-range:w-[370px] sm-range:w-[350px] w-[310px] justify-center group hover:bg-zinc-700 rounded-md cursor-pointer my-3 border ${isDragged ? "border-blue-500" : ""}`} >
            <div className="w-[75px] h-[60px] bg-zinc-700 rounded-md">
                <Image src={image} alt="Image" width={60} height={60} loading="lazy" className="object-cover rounded-md select-none" />
            </div>
